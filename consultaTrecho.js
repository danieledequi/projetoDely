...
function distanciaEntreCidades() {
  ...
  var posicaoCidade2 = cidade.indexOf(document.getElementById("cidadeDestino").value);
  var cidade2 = document.getElementById("cidadeDestino").value;
if (posicaoCidade1 >= 0 & posicaoCidade2 >= 0){
  let result = distancia[posicaoCidade1][posicaoCidade2];
  let precoPorKm;
  ...
  let custoTotal = result * precoPorKm;
  document.getElementById("resultado").innerHTML = 'De ' + cidade1 + ' para ' + cidade2 + ' a distância a ser percorrida é de ' + result + ' km. O custo total da viagem é de R$ ' + custoTotal.toFixed(2) + '.'; 
...
document.getElementById("botao-voltar").addEventListener("click", voltarPaginaAnterior);
} 